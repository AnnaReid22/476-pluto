...
out OUT_struct {
	...
	vec3 fragNor;
	vec2 vTexCoord;
	vec4 fPosLS; //fragment position in light space
	vec3 vColor;
} out_struct;
...
void main() {
  ...
  out_struct.fPos = (M*vec4(vertPos, 1.0)).xyz;
	/* the normal */
  out_struct.fragNor = (M*vec4(vertNor, 0.0)).xyz; //do not want to translate geometry, hence 0.0
  /* pass through the texture coordinates to be interpolated */
  out_struct.vTexCoord = vertTex;
  /* The vertex in light space TODO: fill in appropriately*/
  out_struct.fPosLS = LS*vec4(out_struct.fPos, 1.0); //want to trace geometry, hence 1.0
